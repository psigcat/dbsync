...
class DbTask():
    ...
    def get_last_date(self, service, sensor, sensor_type=1):
        ...
        sql+= " WHERE service_id = "+str(service)+" AND sensor_id = "+str(sensor)   
        if sensor_type <> 1:
            sql+= " AND sensor_type = "+str(sensor_type)
        result = None
        try:         
    ...
    def job_copy_data_1(self, sensor_id, sensor_type=1):
        ...
        # Format previous date (if is different from the default one provided)
        previous_date = self.sensor_dates[sensor_id]
        if str(previous_date) != str(self.default_start_tstamp):
            previous_date = date_to_tstamp(previous_date, "%Y-%m-%d %H:%M:%S", "%Y%m%d%H%M%S")              
        ...
        sql = "SELECT cast(LStepDate as bigint), LStepValue" 
        sql+= " FROM "+self.schema_from+"."+table_from
        sql+= " WHERE LStepDate > "+str(previous_date)+" AND LStepDate < "+str(self.date_to)
        sql+= " ORDER BY LStepDate"
        rows = db_from.get_rows(sql)
        ...
        if rows:
            ...
            if total_found > 0:
                ...
                for row in rows:
                    ...
                    date_in = xstr(row[0]).replace(".", "")
                    date_aux = date_to_tstamp(date_in, "%Y%m%d%H%M%S", "%Y-%m-%d %H:%M:%S")                
            ...
            self.logger.info("Sensor "+str(sensor_id)+" ("+str(sensor_type)+") - Records found:    0") 
    ...
    def job_copy_data_2(self, sensor_id, sensor_type=1):
        ...
        sql+= " WHERE "+col_id+" = "+str(sensor_id)+" AND date > '"+str(previous_date)+"' AND date < '"+str(self.date_to)+"'"
    ...
    def job_copy_data_3(self, sensor_id, sensor_type=1):
        ...
        sql+= " FROM "+self.schema_to+".sensor_"+str(self.service_id)
        sql+= " WHERE id = "+str(sensor_id)
        row = self.db_to.get_row(sql)
        table_from = row[0]
        col_from = row[1]  
        previous_date = self.sensor_dates[sensor_id]
        ...
        # SQL to retrieve data
        sql = "SELECT Time_Stamp, "+col_from
        sql+= " FROM "+self.schema_from+"."+table_from
        sql+= " WHERE Time_Stamp > '"+str(previous_date)+"' AND Time_Stamp < '"+str(self.date_to)+"'"
    ...
    def job_post_process(self, list_insert, audit_table, sensor_id, sensor_type, first_date, last_date, total_found, total_inserted):
        ''' Final step of the task. 
        Insert into 'scada_master' and 'log_detail' tables and commit changes '''
        
        # Complete query with inserted records
        query = ';\n'.join(list_insert)
        query = query+";\n"   
            
        # Add last query: Insert process info into 'log_detail' table
        sql = "INSERT INTO "+audit_table+" (service_id, sensor_id, sensor_type, first_date, last_date, rec_found, rec_inserted) VALUES ("
        log_detail = str(self.service_id)+", "+str(sensor_id)+", "+str(sensor_type)
        log_detail+= ", '"+first_date+"', '"+last_date+"', "+str(total_found)+", "+str(total_inserted)
        sql = sql+log_detail+");"
        self.logger.info("Sensor "+str(sensor_id)+" ("+str(sensor_type)+") - Records inserted: "+str(total_inserted))  
    
        # Execute full query and commit all changes in current thread      
        self.logger.info(sql)  
        query = query+sql
    ...
    def copy_data(self, min_id=-1, max_id=-1, limit=-1, delete_previous_data=0):
        ''' Copy data from origin to destination Database '''                
        
        if delete_previous_data != 0:
        ...
        if model == 1:
            self.date_to = self.date_to+'000000'
        else:
        ...
        if process_numeric_sensors == 1:  
            self.logger.info("{copy_data} \n"+str(sql))                  
            rows = self.db_to.get_rows(sql)  
            self.logger.info("{copy_data} process_numeric_sensors: \n"+str(rows))                
            if rows is not None: 
        ...
        if model == 2 and process_logical_sensors == 1:
            ...
            if rows is not None:  
                ...
                self.logger.info("{copy_data} process_logical_sensors: "+str(rows))                
